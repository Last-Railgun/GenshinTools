...
public class CalUtils {
    ...
    public static Double calBasicDmg(Panel panel, Rate rate, String mode) {
        return panel.getAtk() * rate.getRate() / 100;
    }
    ...
    public static Double calBasicDmg(Panel panel, Rate... rates) {
        ...
        for (Rate rate : rates) {
            switch (rate.getMode()) {
                case "hea":
                    result += rate.getRate() * panel.getHea() / 100;
                    continue;
                case "atk":
                    result += rate.getRate() * panel.getAtk() / 100;
                    continue;
                case "def":
                    result += rate.getRate() * panel.getDef() / 100;
                    continue;
                case "elm":
                    result += rate.getRate() * panel.getElm() / 100;
                    continue;
                default:
